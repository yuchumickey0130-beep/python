...
if st.session_state.game_over:
    ...
    if not df_results.empty:
        ...
        for idx, row in enumerate(last_step_df.itertuples(), start=1):
            mark = " ★ あなた" if row.Player == st.session_state.human_id else ""
...
# --- ゲーム実行中の画面表示 ---
    ...
    # サイドバー表示
    # ------------------------------------------
    st.sidebar.header(f"📌 Step {step} / {NUM_STEPS}")

    # 1. プレイヤー名の設定（Step 1のときのみ変更可能）
    if step == 1:
        new_name = st.sidebar.text_input("👤 プレイヤー名（ID）", value=st.session_state.human_id)
        if new_name and new_name != st.session_state.human_id:
            # 旧プレイヤー名のデータを更新
            old_name = st.session_state.human_id
            st.session_state.human_id = new_name
            st.session_state.players[0] = new_name
            st.session_state.player_cash[new_name] = st.session_state.player_cash.pop(old_name)
            st.rerun()

    human_id = st.session_state.human_id
    my_cash = st.session_state.player_cash[human_id]
    st.sidebar.metric(label=f"現在の{human_id}の現金", value=f"{my_cash:,.1f} 円")

    my_rank = visible_ranks.get(human_id, None)
    rank_disp = f"{my_rank} 位" if my_rank is not None else "非公開"
    st.sidebar.metric(label=f"現在の{human_id}の順位", value=rank_disp)
    ...
    # 2. 強制終了機能（折りたたみ ＋ ダブルチェック構造）
    with st.sidebar.expander("🛠️ 管理用：シミュレーション強制終了"):
        st.caption("途中で切り上げて最終結果画面を表示します。")
